    ...
    public class ObservationViewController : Controller
    {
        ...
        public ActionResult AddObservation()
        {
            var creatures = _context.TblCreatures
                .Select(ur => new SelectListItem { Value = ur.CreatureId.ToString(), Text = ur.ScientificName }).ToList();

            var user = _context.TblUsers
                .Select(k => new SelectListItem { Value = k.UserId.ToString(), Text = k.Name }).ToList();

            var region = _context.TblRegions
                .Select(p => new SelectListItem { Value = p.RegionId.ToString(), Text = p.RegionName }).ToList();

            var migrationstat = _context.TblMigrationstatuses
                .Select(s => new SelectListItem { Value = s.MigrationStatusId.ToString(), Text = s.MigrationStatusName }).ToList();

            var endemicstat = _context.TblEndemicstatuses
                .Select(ss => new SelectListItem { Value = ss.EndemicStatusId.ToString(), Text = ss.EndemicStatus }).ToList();

            var project = _context.TblProjects
                .Select(i => new SelectListItem { Value = i.ProjectId.ToString(), Text = i.ProjectName }).ToList();

            var citation = _context.TblCitations
                .Select(i => new SelectListItem { Value = i.CitationId.ToString(), Text = i.CitationName }).ToList();

            var reference = _context.TblReferences
                .Select(i => new SelectListItem { Value = i.ReferenceId.ToString(), Text = i.ReferenceName }).ToList();

            var locationtype = _context.TblLocationtypes
                .Select(i => new SelectListItem { Value = i.LocationTypeId.ToString(), Text = i.LocationType }).ToList();

            var locationrange = _context.TblLocationranges
                .Select(i => new SelectListItem { Value = i.LocationRangeId.ToString(), Text = i.LocationRangeValue }).ToList();

            var gender = _context.TblGenders
                .Select(i => new SelectListItem { Value = i.GenderId.ToString(), Text = i.GenderName }).ToList();
        ...
        public async Task<IActionResult> GetCitysByRegion(int RegionID)
        {
            var cities = await _context.TblProvinces
                ...
                .Select(k => new SelectListItem { Value = k.ProvinceId.ToString(), Text = k.ProvinceName }).ToListAsync();
        ...
        public async Task<IActionResult> GetDistrictsByCity(int CityID)
        {
            var Districts = await _context.TblDistricts
                ...
                .Select(k => new SelectListItem { Value = k.DistrictId.ToString(), Text = k.DistrictName }).ToListAsync();
        ...
        public async Task<IActionResult> GetLocalitiesByDistrict(int DistrictID)
        {
            var Localities = await _context.TblLocalities
                ...
                .Select(k => new SelectListItem { Value = k.LocalityId.ToString(), Text = k.LocalityName }).ToListAsync();
        ...
        public async Task<IActionResult> GetNeighbourhoodsByLocality(int LocalityID)
        {
            var Neighbourhoods = await _context.TblNeighbourhoods
                ...
                .Select(k => new SelectListItem { Value = k.NeighbourhoodId.ToString(), Text = k.HoodName }).ToListAsync();
        ...
        {
            var filePath = Path.Combine(Directory.GetCurrentDirectory(), "wwwroot/data/TR_iller.geojson");
            var json = System.IO.File.ReadAllText(filePath);
            return Content(json, "application/json");
        }

        // ✅ EKLENEN: Seçilen il adına göre gözlemleri filtreleyen metod
        [HttpGet]
